    ...
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            // 사용자가 플레이 하기를 실행했을 때
                ...
                //2.사용자가 입력한 숫자를 저장할 int변수
                int userNumber = 0;//입력한숫자
                int currentScore = 0;//현재점수
        
                //3.userNumber가 computerNumber와 다를동안 while을 반복한다.
            ...
            //4.사용자의 숫자와 컴퓨터의 숫자를 비교하여 UP혹은 DOWN을 입력한다.
        ...
        //5.최고기록을 확인하여 갱신해야 한다면 갱신한다.
        System.out.printf("현재기록: [%d]회\n",currentScore);
        System.out.printf("최고기록: [%d]회\n",bestScore);
        if(bestScore>currentScore || bestScore==0) {
            //최고기록이 바뀔때는 1.현재점수가 최고점수보다 작을때
//                          2.최고기록이 0일때
            System.out.println("짝짝짝 새로운 최고기록입니다 짝짝짝");
            bestScore = currentScore;
        }
        //플레이기능 종료
    }else if(userChoice == 2) {
        //만약 사용자가 한번이라도 플레이를 하였으면 
        //최고기록은 무조건 0보다 큰수가 된다.
        //따라서 bestScore아 0보다 크면 
        //최고기록을 보여주고 
        //그 외는 아직 플레이 한적이 없음을 알려준다.
        if(bestScore>0) {
            System.out.printf("현재 최고기록: [%d]회\n", bestScore);
            }else {
                System.out.println("아직 플레이기록이 존재하지 않습니다.");
            }
        scanner.close();
    }
}
