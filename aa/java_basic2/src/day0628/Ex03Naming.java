...
//      낙타등 표기법이란, 무언가 이름을 지을때 대소문자를 같이 쓰고 
//      여러단어로 이름을 지을때 두번째 단어부터는 첫글자를 대문자로 적어주는
//      표기법이다. 예시: MemberController, myNumber, Ex03Naming
//  B.뱀 표기법(Snake notation)
//      뱀 표기법이란, 무언가 이름을 지을 때 모두 대문자로 적는 경우에는
//      단어의 사이에_를 넣어서 단어를 구분시키는 방법이다.
//      예시: MY_NUMBER, UNIT_PRICE
//  C. 무언가 이름을 지어줄 때에는 _를 제외한 특수문자는 사용할 수 없다.
//  D. 이름의 중간이나 끝에 숫자가 올 수 있지만 숫자로 이름을 시작할 수 없다.
//  E. 한글로 이름을 지어줄 수 있지만 절.대.로. 하지말자.
//  F. 자바에서는 내부적으로 다른 의미가 실행되는 50가지의 단어들이 있다.
//      해당단어들을 우리가 예약어(KEYWORD)라고 부르는데
//      해당단어들은 이름이 될 수 없다.
//      이클립스에서는 해당 키워드를 자주색으로 표시해준다.
//      만약 여러분들이 어떤이름을 지었는데 자주색으로 표시되면?
 ...
 * 3. unitController 변수
 * 4. unitController() 메소드 
 * 5. unitcontroller 패키지 */
}
