...
public class hwT {
    ...
    private static void insert() {
        ...
        // Employee 객체에 이름 저장
        String empName = ScannerUtil.nextLine(scanner, message);
        e.setEmpName(empName);
        // Employee 객체에 부서 저장
        message = new String("사원의 부서를 입력해주세요.");
        String department = ScannerUtil.nextLine(scanner, message);
        e.setEmpDepartment(department);
        // Employee 객체에 직급 저장
        message = new String("사원의 직급을 입력해주세요.");
        String empRank = ScannerUtil.nextLine(scanner, message);
        e.setEmpRank(empRank);
        // Employee 객체에 연봉 저장
        message = new String("사원의 연봉을 입력해주세요.");
        int empSalary = ScannerUtil.nextInt(scanner, message);
    ...
    private static void printAll() {
        if (list.isEmpty()) {
            ...
            for (int i = 0; i < list.size(); i++) {
                System.out.printf("%d. %s 사원\n", list.get(i).getEmpNumber(), list.get(i).getEmpName());
            }
