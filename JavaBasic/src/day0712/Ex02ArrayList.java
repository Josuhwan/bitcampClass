...
// 하지만, 우리가 직접 경험을 했듯이, ArrayUtil에 만들어놓은 메소드는
// 특정 데이터타입에만 만들면, 매번 해당 데이터타입에 적용될 수 있는
// 우리가 다른 데이터타입을 만들면, 매번 해당 데이터타입에 적용될 수 있는
// 메소드를 ArrayUtil에 처음부터 끝가지 새로 만들었어야 했다.

// 그렇다면, 다바는 동적할당이 가능한 배열을 내부적으로 제공해주지 않을가
// 쉽게 말해서 배열을 클래스화 시켜놓은 것으로써,
// 우리가 데이터타입을 새로 만들더라도
// 이 어레이리스트를 사용하면 손쉽게 동적할당을 적용시킬 수 있다.
...
public class Ex02ArrayList {
	public static void main(String[] args) {
		...
		// < > 안에 적어주어야 한다.
		// 이러한 < >를 우리는 템프릿이라고 부른다.
		// 단 템플릿 안에는 반드시 클래스 이름이 들어와야 한다.
		// 하지만 기본형 데이터타입의 어레이리스트 등
		// 기본형 데이터타입을 모은 클래스가 필요할 때에는
		// 우리가 기본형 데이터타입을 클래스로 "포장" 시켜놓은
		// 자바의 기본 클래스를 사용하면 된다.
		// 대표적으로 int는 Integer, double은 Double 등이 있다.
		// 이러한 클래스들을 "포장 클래스" 혹은 wrapper 클래스 라고 한다.
		
		// 어레이리스트 객체의 선언과 초기화
		// ArrayList<클래스 이름> 이름 = new ArrayList<>();
...
//				get(인덱스)
		...
		System.out.println("list.get(): " + list.get(0).getName());
		...
		// 6. 특정 인덱스에 객체를 다른 객체로 바꿀 때에는?
		//		set()
		System.out.println("6. set()");
		System.out.println("set() 전 get(5).getName(): " + list.get(5).getName());
		Student temp = list.set(5,  s6);
		System.out.println("set() 후 get(5).getName(): " + list.get(5).getName());
		...
		// 8. 특정 객체가 리스트에 존재하는지 확인할때에는
		//    contains().
		//	  단, 객체는 equals()가 true 가 나오는 객체면
		//	  다른 객체더라도 상관 없다.
		...
		System.out.println("list.contains(s100): " + list.contains(s100));
		...
		System.out.println("list.indexOf(s7): " + list.indexOf(s7));
		System.out.println("list.indexOf(s100): " + list.indexOf(s100));
		
		// 10. 특정 객체가 인덱스 중 가장 마지막에 있는 번호를 찾을 때에는?
		//	  lastIndexOf()
		System.out.println("10. lastIndexOf()");
		System.out.println("list.latsIndexOf(s6): " + list.lastIndexOf(s6));
		...
		System.out.println("remove(s7) 전 size(): " + list.size());
		list.remove(s7);
		System.out.println("remove(s7) 후 size(): " + list.size());
		
		System.out.println("remove(s100) 전 size(): " + list.size());
		list.remove(s100);
		System.out.println("remove(s100) 후 size(): " + list.size());
	}
