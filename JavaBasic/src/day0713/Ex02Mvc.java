...
// 로그인 시스템과 게시글 시그템을 융합한다고 가정해보자.
// 만약 단일책임원칙이 정확하게 지켜져있다면,
// 여러분들이 추가적으로 유지보수를 하게 될때 수정해야할 범위가
// 매우 제한적이 되어서 유지보수가 쉬워진다.
// 만약 여러분들이 로그인과 관련된 UI를 수정한다면, 당연히
// 로그인의 UI담당 클래스만 수정하면 된다.
// 하지만 Board02.java처럼 단일책임원칙이 지켜지지 않은 경우에는
...
// V는 Viewer의 줄임말로써, 사용자가 보게 될 화면을 뷰어라고 한다.
// Viewer의 경우, 여러분들이 웹 프로젝트를 진행하게 되면, 웹페이지 그 자체가
// 뷰어를 담당하게 될 수도 있지만, 우리는 콘솔에서 프로그래밍 하기 때문에 클래스로
// 뷰어를 담당하게 될 것이다.
// C는 Controller의 줄임말로써, 사용자가 뷰어에서 요청한 정보를 데이터베이스에서 처리해서, 
// 그 결과를 뷰어로 보내주는 역할을 맡게 된다.
// 우리는 데이터베이스가 존재하지 않기 때문에, 컨트롤러 클래스 안에 private 필드로 
// ArrayList를 하나 만들어서 일종의 데이터베이스처럼 다루게 될 것이다.
