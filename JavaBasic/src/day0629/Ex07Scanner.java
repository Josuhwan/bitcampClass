...
// 스캐너 클래스 뿐만이 아니라 다른 패키지 안의 클래스를 데이터타입으로 만들어야
// 하는 경우에는 매번 우리가 수입 해야 한다.
// 수입은 다음과 같이 이루어 진다.
// import 패키지이름.클래스이름;
...
public class Ex07Scanner {
    public static void main(String[] args) {
        ...
        // Scanner 클래스 변수를 초기화할때에는
        // 다음과 같은 형식을 가져야 한다.
        // 변수이름 = new Scanner(입력받을 위치);
        // 키보드 입력을 받아야 할 때에는
        // 입력받을 위치를 System.in 으로 지정해주면 된다!
        scanner = new Scanner(System.in);

        // 1. int 입력 받기
        System.out.println("1. int 입력 받기");
        ...
        // 2. double 입력 받기
        System.out.println("2. double 입력 받기");
        ...
        // next() 혹은 nextLine()을 쓰게 되는데
        // next()의 경우, 스페이스같은 일반적인 공백을 쓰면
        // 그 공백 이전까지의 내용만 입력이 되지만
        ...
        // nextLine()의 경우
        // 단, nextLine()을 사용할 때 한가지 주의할 점이 있다.
        // nextInt(), nextDouble()등의 숫자를 입력받는 메소드 이후에
        // nextLine()을 사용하게 되면, Scanner 클래스의 버퍼 메모리를 읽어오는 방식에서
        // 발생하는 버그 때문에
        // 실제론 아무런 값도 입력해줄 수 없는 상태가 된다.
        // 이러한 버그를 해결하는 방법은 매우 간단한다.
        ...
        // scanner.nextLine()도 입력을 받기 위해 대기하는 상태가 되어버리기
        // 때문이다!
        ...
        // 반드시 프로그램 제일 마지막에 close() 메소드를 실행해주는 것이
        // 좋다.
        scanner.close();
    }
}
